...
public abstract class Game {
	...
	for (Map.Entry<Integer, SpriteContent> entry : entries) {
		...
		VGDLSprite resourceTest = VGDLFactory.GetInstance().createSprite(entry.getValue(), new Vector2d(0, 0),
			new Dimension(1, 1));
	...
	data.isSingleton = false;
	if(sc.parameters.get("singleton") != null){
	...
	if (isHuman && !wi.windowClosed && CompetitionParameters.killWindowOnEnd) {
	    if (CompetitionParameters.dialogBoxOnStartAndEnd) {
		if (no_players == 1) {
		    JOptionPane.showMessageDialog(frame, "GAMEOVER: YOU "
	...
	while (timeEffects.size() > 0 && timeEffects.first().nextExecution <= gameTick) {
		...
		for (Integer itype : allTypes) {
			...
			// Check that they are not dead (could happen in this
			// same cycle).
	    ...
	    // If the time effect is repetitive, need to reinsert in the list of
	    // effects
	    if (ef.repeating) {
		if (!exec)
	...
	for (Integer intId : definedEOSEffects) {
		...
		for (Integer itype : allTypes) {
		    ...
		    Collection<VGDLSprite> sprites = this.getSprites(itype);
		    for (VGDLSprite sp : sprites) {
			// Check if they are at the edge to trigger the effect.
	...
	for (Pair<Integer, Integer> p : definedEffects) {
		...
		// Consider the two types to populate the array bucketList, that
		// encloses the sprites
		// of both types that could take part in any interaction.
		int[] types = (getNumSprites(p.first) < getNumSprites(p.second)) ? new int[] { p.first, p.second }
			: new int[] { p.second, p.first };

		// store all sprites of the type that we have the lowest number
		// of sprites of in bucketList
		int intId = types[0];

		// we'll mark here the rows of the map that could possibly
		// collide with rows of the map
		// where we find sprites of the first type. Sprites of the
		// second type will only be added
		// to buckets that have been marked as important, or are outside
		// of the map bounds
		...
		// if we have found sprites for the first type, also search for
		// sprites of the second type
		if (!noSprites[intId]) {
		    intId = types[1];
		    if (!noSprites[intId] && bucketList[intId].size() == 0) {
			// Take all the subtypes in the hierarchy of this
			// sprite.
			ArrayList<Integer> allTypes = iSubTypes[intId];
			for (Integer itype : allTypes) {
			    Collection<VGDLSprite> sprites = this.getSprites(itype);
			    for (VGDLSprite sp : sprites) {
				// only add this sprite if he's outside the map
				// bounds, or if he is in an important
				// bucket (one that could possibly collide with
				// sprites added of the first type)
		...
		// Take the collections of sprites, one for each type, of the
		// two sprite types of this effect.
		...
		for (Integer bucket1 : first.keySet()) {
		    // For each bucket with sprites of this collision pair, get
		    // the sprites of p.first.
			...
			for (VGDLSprite s1 : sprites1nBucket1) {
				...
				if (s1.bucketSharp)
				    ...
				    buckets = new int[] { s1.bucket, s1.bucket + 1 };

				for (int bucketId : buckets) {
				    // On each bucket, take the sprites if the
				    // p.second sprite type.
					...
					s2loop: for (int idx2 = 0; idx2 < numSprites2; idx2++) {
					    // Take each sprite of p.second and
					    // check for collision
	...
	if (!anyother) {
	    ...
	    if (templateSprites[itype] == null) // don't have a template yet, so
						// need to create one
	    {
		newSprite = VGDLFactory.GetInstance().createSprite(content, position,
